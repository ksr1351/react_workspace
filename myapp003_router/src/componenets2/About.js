...
const About = () => {
  ...
  const location = useLocation(); //이 코드를 쓸때는 querystring 안써도 됨 (<h5>pathname: {location.pathname}</h5> <h5>search : {location.search}</h5>)

  const { search, pathname } = useLocation(); //이 코드와 아래 코드쓰려면 querystring 다운로드 받기 <h5>pathname: {pathname}</h5><h5>search: {search}</h5>
